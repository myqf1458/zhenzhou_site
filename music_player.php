...
/**
 * 悬浮音乐播放器插件
 * 使用方式：在任意页面底部添加 <?php include 'music_player.php'; ?>
 * 需要配合 music_api.php 使用
 */
?>
<!-- ===== 悬浮音乐播放器插件 ===== -->
...
<style>
    ...
    @keyframes mpBarPulse {
        0%, 100% { height: 6px; }
        50% { height: 14px; }
    }
    @keyframes mpVinylSpin {
        from { transform: rotate(0deg); }
        to { transform: rotate(360deg); }
    }
    ...
    #mpTrigger:hover {
        transform: scale(1.1);
        box-shadow: 0 6px 28px rgba(201,169,212,0.5);
    }
    #mpTrigger i {
        font-size: 20px;
        color: #fff;
    ...
    #mpTrigger .mp-bars span {
        width: 3px;
        background: #fff;
        border-radius: 2px;
        animation: mpBarPulse 0.6s ease-in-out infinite;
    }
    ...
    .mp-header {
        ...
        gap: 14px;
        padding: 16px;
        background: linear-gradient(135deg, rgba(201,169,212,0.12), rgba(232,197,208,0.12));
    }
    .mp-cover-wrap {
        position: relative;
        width: 64px;
    ...
    .mp-cover {
        ...
        object-fit: cover;
        background: linear-gradient(135deg, #c9a9d4, #e8c5d0);
        box-shadow: 0 2px 12px rgba(201,169,212,0.3);
    }
    .mp-cover-wrap.playing .mp-cover {
        animation: mpVinylSpin 8s linear infinite;
    }
    .mp-cover-wrap::after {
        content: '';
        position: absolute;
        ...
        border-radius: 50%;
        background: #fff;
        box-shadow: 0 0 0 2px rgba(201,169,212,0.3);
    }
    .mp-info {
        flex: 1;
        min-width: 0;
    ...
    .mp-progress-filled {
        height: 100%;
        background: linear-gradient(90deg, #c9a9d4, #e8c5d0);
        border-radius: 2px;
        width: 0%;
        position: relative;
        transition: width 0.1s linear;
    }
    .mp-progress-filled::after {
        content: '';
        position: absolute;
        right: -5px;
        top: 50%;
        ...
        border-radius: 50%;
        background: #fff;
        box-shadow: 0 1px 4px rgba(201,169,212,0.4);
        opacity: 0;
    ...
    .mp-btn-play:hover {
        background: linear-gradient(135deg, #b892c2, #d8b5c0);
        color: #fff;
    ...
    .mp-volume-filled {
        height: 100%;
        background: rgba(201,169,212,0.5);
        border-radius: 2px;
        width: 80%;
    }
    ...
    .mp-playlist-toggle {
        text-align: center;
        padding: 8px;
        border-top: 1px solid rgba(201,169,212,0.15);
        cursor: pointer;
        font-size: 12px;
        color: #8b8594;
        transition: all 0.2s;
    ...
    .mp-playlist-item .pl-thumb {
        width: 32px; height: 32px;
        border-radius: 8px;
        object-fit: cover;
        flex-shrink: 0;
    }
    .mp-playlist-item .pl-info {
        flex: 1; min-width: 0;
    }
    .mp-playlist-item .pl-title {
        font-size: 13px;
        color: #4a4556;
        white-space: nowrap;
        overflow: hidden;
    ...
    .mp-playlist-item .pl-artist {
        font-size: 11px;
        color: #b5b0bc;
    }
    .mp-playlist-item .pl-status {
        font-size: 12px;
        color: #c9a9d4;
        width: 16px;
        text-align: center;
...
<div id="mpPanel">
    <div class="mp-header">
        <div class="mp-cover-wrap" id="mpCoverWrap">
            <img class="mp-cover" id="mpCover" src="" alt="封面" onerror="this.src='https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=' + encodeURIComponent('anime music note cute pastel pink purple album cover') + '&image_size=square'">
        </div>
        <div class="mp-info">
    ...
    <div class="mp-playlist-toggle" id="mpPlaylistToggle">
        播放列表 <i class="fas fa-chevron-down"></i>
    </div>
...
(function() {
    ...
    var currentIndex = -1;
    var isPlaying = false;
    var playMode = 'list'; // list: 列表循环, single: 单曲循环, random: 随机
    var volume = 0.8;
    var isMuted = false;
    var lastVolume = 0.8;

    // ===== DOM 元素 =====
    var trigger = document.getElementById('mpTrigger');
    ...
    var coverWrap = document.getElementById('mpCoverWrap');
    ...
    var modeBtn = document.getElementById('mpModeBtn');
    var progressBar = document.getElementById('mpProgressBar');
    var progressFilled = document.getElementById('mpProgressFilled');
    var currentTimeEl = document.getElementById('mpCurrentTime');
    var durationEl = document.getElementById('mpDuration');
    var volumeBar = document.getElementById('mpVolumeBar');
    var volumeFilled = document.getElementById('mpVolumeFilled');
    var volumeIcon = document.getElementById('mpVolumeIcon');
    var playlistToggle = document.getElementById('mpPlaylistToggle');
    ...
    function showToast(msg) {
        ...
        t.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.7);color:#fff;padding:10px 24px;border-radius:12px;font-size:14px;z-index:10001;pointer-events:none;animation:mpSlideIn 0.2s ease;';
        t.textContent = msg;
    ...
    function renderPlaylist() {
        ...
        playlist.forEach(function(song, i) {
            ...
            html += '<img class="pl-thumb" src="' + song.cover + '" onerror="this.style.opacity=\'0.3\'" alt="">';
            html += '<div class="pl-info">';
            html += '<div class="pl-title">' + escapeHtml(song.title) + '</div>';
            html += '<div class="pl-artist">' + escapeHtml(song.artist) + '</div>';
            html += '</div>';
    ...
    function playNext() {
        ...
        if (playMode === 'random') {
            ...
            next = currentIndex; // 单曲循环由 ended 事件处理
        } else {
    ...
    modeBtn.addEventListener('click', function() {
        ...
        updateModeUI();
        var names = { list: '列表循环', single: '单曲循环', random: '随机播放' };
        showToast(names[playMode]);
    });
